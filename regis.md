...
开启主从复制后，主机有的数据，从机会复制一份，不管新旧。

被当做从机的redis不能有写入操作，主机死后，从机会原地待命。从机死后，重启时需要重新配置从机，配置好redis.conf文件后，可以在重启从机时自动配置。

slave启动后，从机会向主机发送一个sync命令，首次复制是全量复制，后续为增量复制。重新连接时，也会执行一次全两父子。
...
一主一从，第二台从机配置的ip为第一台从机，以此推类。

##### 容灾方案

直接取消配置，一般来说，这种配置在链式配置时最好，主机嗝屁后，直接上紧跟着的从机，其他从机就不用配置了。

##### 哨兵模式

需要建立一个sentinel.conf文件
...
```shell
# 名称随便取 ，最后的1是票数，决定主机死后谁自动上位
sentinel monitor 被监控数据库名称 127.0.0.1 6379 1
```
